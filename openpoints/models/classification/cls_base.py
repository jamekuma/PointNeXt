...
class BaseCls_Resampling(nn.Module):
    def __init__(self,
        ...
        load_pretrain = kwargs.get('load_pretrain', None)
        train_only_resampler = kwargs.get('train_only_resampler', False)
        
        if load_pretrain is not None:
            ckpt_dict = torch.load(load_pretrain)["model"]
            # ckpt_dict = {k[len("encoder."):]: v for k, v in ckpt_dict.items()}
        ...
        if train_only_resampler:
            for n, p in self.named_parameters():
                if "resampler" not in n:
